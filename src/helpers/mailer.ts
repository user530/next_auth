...
export const sendEmail = async (mailData: IMailData) => {
    try {
        ...
        const mailOptions = {
            ...
            html: `<p>Click <a href="${process.env.DOMAIN}/${isVerifyReq ? 'verify_email' : 'reset_email'}?token=${hashedToken}">here</a> to ${isVerifyReq ? 'verify your email' : 'reset your password'}</p>`
        };
